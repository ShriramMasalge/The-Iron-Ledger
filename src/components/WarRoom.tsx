...
export default function WarRoom({ account, trades, onBack, onTradeUpdate }: WarRoomProps) {
  ...
  const [mobileTab, setMobileTab] = useState<'queue'|'log'>('queue');
  ...
  const active = [...trades]
    ...
    .sort((a,b)=>{
      ...
      if(aOD&&!bOD) return -1; if(!aOD&&bOD) return 1;
      return a.deadline-b.deadline;
    });

  const overdue = active.filter(t=>t.deadline<now&&['Funded','InTransit'].includes(t.state)&&!t.sellerSlashed);
  const totalLocked = trades.filter(t=>['Funded','InTransit','Delivered'].includes(t.state)).reduce((s,t)=>s+BigInt(t.amount),BigInt(0));
  ...
  const executeSlash = async (trade: Trade) => {
    ...
    try {
      ...
      setError(msg); addLog(`❌ Failed: ${msg}`, C.danger);
    } finally { setSlashing(null); }
  };
  ...
  const MobileTradeCard = ({ trade }: { trade: Trade }) => {
    ...
    return (
      <div style={{ background:isOverdue?'rgba(255,53,53,0.025)':C.surface, border:`1px solid ${isOverdue?'rgba(255,53,53,0.25)':C.border}`, borderRadius:8, padding:'14px 16px', marginBottom:10, transition:'border-color 0.3s' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
          <div>
            <div style={{ fontSize:10, color:C.dim, fontFamily:C.mono, marginBottom:3 }}>{trade.id.slice(0,10)}…{trade.id.slice(-4)}</div>
            <div style={{ fontSize:16, color:C.accent, fontFamily:C.mono, fontWeight:700 }}>{formatEth(trade.amount)} ETH</div>
          </div>
          <span style={{ fontSize:9, padding:'3px 9px', background:`${stateColor[trade.state]}15`, border:`1px solid ${stateColor[trade.state]}33`, borderRadius:100, color:stateColor[trade.state], fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase' }}>
            {trade.state}
          </span>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:10 }}>
          <div>
            <div style={{ fontSize:8, letterSpacing:'0.15em', color:C.dim, textTransform:'uppercase', marginBottom:2 }}>Penalty</div>
            <div style={{ fontSize:12, color:C.orange }}>{(trade.slashingPenaltyBps/100).toFixed(2)}% = {(parseFloat(formatEth(trade.amount))*trade.slashingPenaltyBps/10000).toFixed(5)} ETH</div>
          </div>
          <div>
            <div style={{ fontSize:8, letterSpacing:'0.15em', color:C.dim, textTransform:'uppercase', marginBottom:2 }}>Time Left</div>
            <TimeLeft deadline={trade.deadline} />
          </div>
        </div>
        {['Funded','InTransit','Delivered'].includes(trade.state) && !isOverdue && (
          <CountdownBar deadline={trade.deadline} createdAt={trade.createdAt} />
        )}
        {canSlash && (
          <button onClick={()=>executeSlash(trade)} disabled={isSlashing||slashing!==null}
            style={{ width:'100%', padding:'13px', marginTop:10, background:'rgba(255,53,53,0.08)', border:'1px solid rgba(255,53,53,0.4)', color:C.danger, borderRadius:6, cursor:isSlashing?'wait':'pointer', fontFamily:C.mono, fontSize:11, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', minHeight:'46px', animation:!isSlashing&&slashing===null?'wrPulse 1.6s ease-in-out infinite':'none' }}>
            {isSlashing ? '⏳ Slashing…' : `⚡ SLASH — SEIZE ${(trade.slashingPenaltyBps/100).toFixed(2)}%`}
          </button>
        )}
        {isOverdue && !canSlash && !trade.sellerSlashed && (
          <div style={{ marginTop:8, fontSize:10, color:'rgba(255,53,53,0.5)' }}>OVERDUE — you are not the buyer</div>
        )}
        {trade.sellerSlashed && (
          <div style={{ marginTop:8, fontSize:9, color:C.dim }}>SLASHED ✓</div>
        )}
      </div>
    );
  };
  ...
  return (
    <>
      ...
      <div style={{ minHeight:'100vh', background:C.bg, color:C.text, fontFamily:C.mono, display:'flex', flexDirection:'column', animation:'wrFadeIn 0.35s ease' }}>
        ...
        {/* Top bar */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding: isMobile?'10px 14px':'10px 20px', borderBottom:`1px solid ${C.border}`, background:'rgba(0,0,0,0.35)', backdropFilter:'blur(8px)', position:'sticky', top:0, zIndex:50, flexWrap:'wrap', gap:10 }}>
          <div style={{ display:'flex', alignItems:'center', gap:isMobile?10:16 }}>
            <button onClick={onBack} style={{ background:'transparent', border:`1px solid ${C.border}`, color:C.dim, padding: isMobile?'7px 12px':'6px 14px', borderRadius:4, cursor:'pointer', fontFamily:C.mono, fontSize:10, letterSpacing:'0.1em', textTransform:'uppercase', minHeight:'36px' }}>
              ← {isMobile?'':'Ledger'}
            </button>
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <span style={{ fontSize: isMobile?10:11, fontWeight:700, letterSpacing:'0.15em', color:overdue.length>0?C.danger:C.accent }}>RISK MONITOR</span>
              </div>
              {!isMobile && <div style={{ fontSize:9, color:C.dim, letterSpacing:'0.1em' }}>LIVE RISK MONITORING</div>}
            </div>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:isMobile?10:16, flexWrap:'wrap' }}>
            {[
              { label:'OD',     value:overdue.length,                                                              color:overdue.length>0?C.danger:C.dim },
              { label:'Active', value:active.length,                                                               color:C.accent },
              { label:'Locked', value:parseFloat(ethers.utils.formatEther(totalLocked.toString())).toFixed(2)+' E', color:C.blue },
            ].map(s=>(
              <div key={s.label} style={{ textAlign:'center' }}>
                <div style={{ fontSize:isMobile?16:18, fontFamily:C.serif, fontWeight:700, color:s.color, lineHeight:1 }}>{s.value}</div>
                <div style={{ fontSize:7, letterSpacing:'0.15em', color:C.dim, textTransform:'uppercase', marginTop:2 }}>{s.label}</div>
              </div>
            ))}
            {overdue.length>1 && (
              <button onClick={slashAll} disabled={slashing!==null}
                style={{ padding:'7px 14px', background:'rgba(255,53,53,0.08)', border:'1px solid rgba(255,53,53,0.4)', color:C.danger, borderRadius:4, cursor:slashing!==null?'not-allowed':'pointer', fontFamily:C.mono, fontSize:10, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', animation:slashing===null?'wrPulse 1.6s ease-in-out infinite':'none', minHeight:'36px' }}>
        ...
        {/* Summary stats — horizontal strip */}
        <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:1, background:C.border, borderBottom:`1px solid ${C.border}` }}>
          {[
            ...
            { label:'Transit',  value:trades.filter(t=>t.state==='InTransit').length, color:C.orange },
            { label:'Slashed',  value:trades.filter(t=>t.sellerSlashed).length,   color:C.danger  },
            { label:'Complete', value:trades.filter(t=>t.state==='Completed').length, color:'#00cc77' },
          ].map(({label,value,color})=>(
            <div key={label} style={{ background:C.bg, padding: isMobile?'10px 6px':'12px 16px', textAlign:'center' }}>
              <div style={{ fontSize:isMobile?16:18, fontFamily:C.serif, fontWeight:700, color, lineHeight:1, marginBottom:3 }}>{value}</div>
              <div style={{ fontSize:8, letterSpacing:'0.14em', color:C.dim, textTransform:'uppercase' }}>{label}</div>
            </div>
          ))}
        </div>
        ...
        {/* ── Mobile: tabs ── */}
        {isMobile && (
          <div style={{ display:'flex', borderBottom:`1px solid ${C.border}`, background:C.surface, position:'sticky', top:'56px', zIndex:40 }}>
            {(['queue','log'] as const).map(t=>(
              <div key={t} onClick={()=>setMobileTab(t)} style={{ flex:1, padding:'11px', textAlign:'center', fontSize:9, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', color:mobileTab===t?C.accent:C.dim, borderBottom:mobileTab===t?`2px solid ${C.accent}`:'2px solid transparent', cursor:'pointer', transition:'all 0.15s' }}>
                {t==='queue'?`Queue (${active.length})`:'Log'}
              </div>
            ))}
          </div>
        )}
        ...
        {/* Main area */}
        <div style={{ flex:1, display: isMobile?'block':'grid', gridTemplateColumns:isMobile?undefined:'1fr 268px', overflow:'hidden' }}>

          {/* Trade queue */}
          {(!isMobile || mobileTab === 'queue') && (
            <div style={{ overflowY:'auto', padding: isMobile?'14px 14px 80px':'20px 24px', borderRight: isMobile?'none':`1px solid ${C.border}` }}>
              ...
              {lastTx && <div style={{ display:'flex', justifyContent:'space-between', background:'rgba(184,255,0,0.04)', border:'1px solid rgba(184,255,0,0.2)', borderRadius:4, padding:'10px 14px', fontSize:12, color:C.accent, marginBottom:14, flexWrap:'wrap', gap:8 }}><span>✓ Slashing executed</span><span style={{ fontSize:10, color:C.dim }}>{lastTx.slice(0,18)}…</span></div>}
              ...
              {/* Mobile: card layout */}
              {isMobile && active.map(trade => <MobileTradeCard key={trade.id} trade={trade} />)}
                    ...
                    return (
                      <div key={trade.id} style={{ background:isOverdue?'rgba(255,53,53,0.025)':C.surface, border:`1px solid ${isOverdue?'rgba(255,53,53,0.25)':C.border}`, borderRadius:6, padding:'12px 14px', marginBottom:8, transition:'border-color 0.3s' }}>
                        <div style={{ display:'grid', gridTemplateColumns:'1fr 80px 88px 80px 90px 148px', gap:8, alignItems:'center' }}>
                          ...
                          <div style={{ fontSize:12, color:C.accent, fontFamily:C.mono, fontWeight:600 }}>{formatEth(trade.amount)}<span style={{ fontSize:9, color:C.dim }}> ETH</span></div>
                          <div>
                          ...
                          <div>
                            <span style={{ fontSize:9, padding:'2px 8px', background:`${stateColor[trade.state]}15`, border:`1px solid ${stateColor[trade.state]}33`, borderRadius:100, color:stateColor[trade.state], fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', whiteSpace:'nowrap' }}>{trade.state}</span>
                          </div>
          ...
          {(!isMobile || mobileTab === 'log') && (
            <div style={{ display:'flex', flexDirection:'column', background:C.surface, overflow:'hidden', minHeight: isMobile?'calc(100vh - 160px)':'unset' }}>
