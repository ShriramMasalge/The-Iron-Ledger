...
function DeadlineRingPreview({ seconds, demoMode }: { seconds: number; demoMode: boolean }) {
  ...
  useEffect(() => {
    ...
    }, demoMode ? 100 : 1000); // fast preview in demo mode
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      <div style={{ position: 'relative', width: 140, height: 140 }}>
        <svg width="140" height="140" style={{ transform: 'rotate(-90deg)' }}>
          <circle cx="70" cy="70" r={R} fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="4" />
          <circle
            cx="70" cy="70" r={R}
            fill="none"
            stroke={color}
        ...
        <div style={{
          ...
          {overdue ? (
            ...
            <>
              <span style={{ fontSize: elapsed > 0 ? 18 : 14, color, fontWeight: 700, fontFamily: C.mono }}>
                {elapsed > 0 ? fmt(remaining) : fmt(seconds)}
              </span>
              <span style={{ fontSize: 8, color: C.dim, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
                {elapsed > 0 ? 'remaining' : 'total'}
              </span>
            </>
          )}
        </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          ...
          style={{
            padding: '6px 14px',
            background: running ? 'rgba(184,255,0,0.04)' : 'rgba(184,255,0,0.12)',
            border: `1px solid rgba(184,255,0,${running ? '0.1' : '0.3'})`,
            borderRadius: 4,
            color: running ? 'rgba(184,255,0,0.3)' : C.accent,
            fontSize: 10,
            fontFamily: C.mono,
            letterSpacing: '0.1em',
      ...
      <div style={{ fontSize: 10, color: C.dim, textAlign: 'center', lineHeight: 1.6 }}>
        {demoMode
          ? 'Demo: ring drains fast for preview.\nReal countdown is 1:1 seconds.'
          : 'Ring drains over the full deadline period.'}
      </div>
    </div>
  );
}
...
function PenaltyDial({ bps, onChange }: { bps: number; onChange: (v: number) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
      ...
      <input
        type="range" min="10" max="255" value={bps}
...
function ContractPreview({
  seller, amountEth, deadlineSecs, penaltyBps, demoMode, account
}: {
  seller: string; amountEth: string; deadlineSecs: string;
  penaltyBps: number; demoMode: boolean; account: string;
}) {
  ...
  const amountWei = amountEth && !isNaN(Number(amountEth))
    ? (() => { try { return ethers.utils.parseEther(amountEth).toString(); } catch { return '—'; } })()
    : '—';
  const penaltyEth = amountEth && !isNaN(Number(amountEth)) && !isNaN(penaltyBps)
    ? (Number(amountEth) * penaltyBps / 10000).toFixed(6)
    : '—';

  const line = (label: string, value: string, highlight?: boolean) => (
    <div key={label} style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 8, padding: '5px 0', borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 9, letterSpacing: '0.2em', color: C.accent, textTransform: 'uppercase', marginBottom: 12 }}>
        ▸ CONTRACT PREVIEW
      </div>
      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)', marginBottom: 12, fontStyle: 'italic' }}>
        IronLedger.createTrade(...)
      </div>
      {line('contract', CONTRACT_ADDRESS.slice(0,10) + '…' + CONTRACT_ADDRESS.slice(-6))}
      ...
      {line('amount (wei)', amountWei)}
      {line('deadline', deadlineSecs ? deadlineStr : '—')}
      {line('deadline mode', demoMode ? 'DEMO (seconds)' : 'PRODUCTION (days)')}
      ...
      <div style={{ marginTop: 14, padding: '8px 10px', background: 'rgba(184,255,0,0.03)', border: '1px solid rgba(184,255,0,0.1)', borderRadius: 4, fontSize: 10, color: C.dim, lineHeight: 1.7 }}>
        If seller misses deadline, buyer may execute slashing and seize <span style={{ color: C.accent }}>{penaltyEth} ETH</span> from escrow. Remaining funds are refunded to buyer.
      </div>
    </div>
  );
}

// ── Recent trade mini-row ─────────────────────────────────────
function RecentRow({ trade }: { trade: Trade }) {
  ...
  return (
    <div style={{ padding: '9px 0', borderBottom: `1px solid ${C.border}`, display: 'grid', gridTemplateColumns: '1fr auto', gap: 8, alignItems: 'center' }}>
      <div>
        <div style={{ fontSize: 10, color: C.dim, fontFamily: C.mono, marginBottom: 2 }}>
      ...
      <div style={{ textAlign: 'right' }}>
        <span style={{
          ...
          borderRadius: 100, color: c,
          fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase',
          fontFamily: C.mono, display: 'block', marginBottom: 3,
        }}>{trade.state}</span>
        {overdue && <span style={{ fontSize: 8, color: C.danger }}>⚡ OVERDUE</span>}
      </div>
    </div>
  );
}
...
function GasEstimate({ amountEth }: { amountEth: string }) {
  ...
  const totalEth = amountEth && !isNaN(Number(amountEth))
    ? (Number(amountEth) + Number(gasCostEth)).toFixed(6) : '—';
  ...
  return (
    <div style={{ padding: '12px 14px', background: 'rgba(56,189,248,0.04)', border: '1px solid rgba(56,189,248,0.15)', borderRadius: 6, fontSize: 11 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 12px', fontFamily: C.mono }}>
        <span style={{ color: C.dim, fontSize: 10 }}>Gas Units</span>
        <span style={{ color: C.text, fontSize: 10 }}>{gasUnits.toLocaleString()}</span>
        <span style={{ color: C.dim, fontSize: 10 }}>Gas Price</span>
        <span style={{ color: C.text, fontSize: 10 }}>{gasPriceGwei} gwei</span>
        <span style={{ color: C.dim, fontSize: 10 }}>Gas Cost</span>
        <span style={{ color: C.blue, fontSize: 10 }}>{gasCostEth} ETH (≈${gasCostUsd})</span>
        <span style={{ color: C.dim, fontSize: 10 }}>Total (incl gas)</span>
        <span style={{ color: C.accent, fontSize: 10, fontWeight: 700 }}>{totalEth} ETH</span>
      </div>
      <div style={{ fontSize: 9, color: C.dim, marginTop: 8 }}>* Estimate only. Actual gas varies.</div>
    </div>
  );
}

// ═══════════════════════════════════════════════════════════════
// THE FORGE — main export
// ═══════════════════════════════════════════════════════════════
export default function TheForge({ account, trades, onBack, onCreated }: ForgeProps) {
  ...
  const lbl: React.CSSProperties = {
    ...
    letterSpacing: '0.15em', color: C.dim, textTransform: 'uppercase', marginBottom: 8,
  };
  ...
  const createTrade = async () => {
    if (!seller || !amountEth || !deadlineSecs) {
      setError('Seller address, amount and deadline are required.');
      return;
    }
    try {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <div style={{ fontSize: 9, letterSpacing: '0.2em', color: C.accent, textTransform: 'uppercase', marginBottom: 2 }}>
                Iron Ledger · Screen 03
              </div>
              <div style={{ fontSize: 18, fontFamily: C.serif, color: '#f2ede6', fontWeight: 700, letterSpacing: '-0.02em' }}>
                The Forge
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 9, color: C.dim, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                {demoMode ? 'Demo (secs)' : 'Production (days)'}
              </span>
              <div
                onClick={() => { setDemoMode(d => !d); setDeadlineSecs(demoMode ? '7' : '60'); }}
                style={{
                  width: 32, height: 17, borderRadius: 100,
                  background: demoMode ? 'rgba(184,255,0,0.2)' : 'rgba(255,255,255,0.06)',
                  border: `1px solid ${demoMode ? 'rgba(184,255,0,0.4)' : C.border}`,
              ...
              >
                <div style={{
                  ...
                  background: demoMode ? C.accent : C.dim,
        ...
        <div style={{
          ...
          <div style={{ background: C.bg, padding: '28px 24px', overflowY: 'auto' }}>
            <div style={{ fontSize: 9, letterSpacing: '0.2em', color: C.dim, textTransform: 'uppercase', marginBottom: 20 }}>
              ▸ FORGE PARAMETERS
            </div>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ fontSize: 10, color: C.dim, marginTop: 5 }}>
                Counterparty who will receive funds on completion.
              </div>
            </div>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              />
              {amountEth && !isNaN(Number(amountEth)) && (
                <div style={{ fontSize: 10, color: C.accent, marginTop: 5 }}>
                  ≈ ${(Number(amountEth) * 3820).toFixed(2)} USD
                </div>
              )}
            </div>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ fontSize: 10, color: C.dim, marginTop: 5 }}>
                {demoMode
                  ? '⚡ Demo: seconds. Set 30 for instant slash demo.'
                  : '🏭 Production: calendar days until delivery.'}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ fontSize: 10, color: C.dim, marginTop: 8, lineHeight: 1.6 }}>
                ...
                {amountEth && !isNaN(Number(amountEth))
                  ? `= ${(Number(amountEth) * penaltyBps / 10000).toFixed(6)} ETH penalty`
                  : 'Enter amount to see ETH value.'}
              </div>
            </div>
            ...
            {error && (
              <div style={{
                ...
                background: 'rgba(255,53,53,0.06)', border: '1px solid rgba(255,53,53,0.2)',
                borderRadius: 4, fontSize: 12, color: '#ff7070',
              }}>
                {error}
              </div>
            )}
            {success && (
              <div style={{
                padding: '10px 14px', marginBottom: 14,
                background: 'rgba(184,255,0,0.05)', border: '1px solid rgba(184,255,0,0.2)',
                borderRadius: 4, fontSize: 12, color: C.accent,
              }}>
                ✓ {success}
                {txHash && (
            ...
            <button
              ...
              style={{
                ...
                background: loading ? 'rgba(184,255,0,0.04)' : C.accent,
                color: loading ? C.accent : C.bg,
                border: loading ? `1px solid rgba(184,255,0,0.2)` : 'none',
            ...
            >
              {!account ? 'Wallet Not Connected' : loading ? '⏳ Submitting to Chain…' : '⚒ Forge Trade Contract'}
            </button>

            {!account && (
              <div style={{ fontSize: 10, color: C.dim, textAlign: 'center', marginTop: 8 }}>
                Connect wallet from the main screen to submit.
              </div>
            )}
          </div>
          ...
          <div style={{ background: C.panel, padding: '28px 28px', overflowY: 'auto' }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 9, letterSpacing: '0.2em', color: C.dim, textTransform: 'uppercase', marginBottom: 4 }}>
                COUNTDOWN RING PREVIEW
              </div>
              <DeadlineRingPreview
                seconds={Number(deadlineSecs) || 60}
                demoMode={demoMode}
              />
            </div>
            ...
            {demoMode && (
              <div style={{
                ...
                <div style={{ color: C.accent, fontWeight: 700, marginBottom: 6, fontSize: 10, letterSpacing: '0.1em' }}>
                  ⚡ DEMO WORKFLOW
                </div>
                1. Set deadline to <strong style={{ color: C.accent }}>30</strong> seconds<br />
                2. Click <strong style={{ color: C.accent }}>Forge Trade Contract</strong><br />
                3. Fund the trade in the Ledger<br />
                4. Watch the ring drain to zero<br />
                5. Slashing protocol arms automatically →<br />
                &nbsp;&nbsp;&nbsp;<strong style={{ color: C.danger }}>⚡ Execute Slashing Penalty</strong>
              </div>
            )}
          </div>
          ...
          <div style={{ background: C.bg, padding: '28px 20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 24 }}>
            {/* Gas estimate */}
            <div>
              <div style={{ fontSize: 9, letterSpacing: '0.2em', color: C.dim, textTransform: 'uppercase', marginBottom: 14 }}>
                ▸ COST BREAKDOWN
              </div>
            ...
            <div style={{ flex: 1 }}>
              ...
              {recentTrades.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px 16px', color: C.dim, fontSize: 12, lineHeight: 1.7 }}>
                  No trades yet.<br />
                  <span style={{ fontSize: 10 }}>Your history will appear here after your first forge.</span>
                </div>
              ) : (
            ...
            {trades.length > 0 && (
              <div style={{
                ...
                <div style={{ fontSize: 9, letterSpacing: '0.2em', color: C.dim, textTransform: 'uppercase', marginBottom: 12 }}>
                  ▸ YOUR STATS
                </div>
                {[
