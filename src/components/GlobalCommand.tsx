...
// ── Static globe nodes (world map positions in SVG 0-800 x 0-400) ──
          ...
          >
            ...
            {(isPulsing || isHovered) && (
              <circle
                ...
                r={r + 6}
                fill="none"
                stroke={node.active ? C.accent : C.orange}
                strokeWidth="1"
                opacity="0.4"
...
// ── Stat Block ────────────────────────────────────────────────
function StatBlock({ label, value, sub, accent, large }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 9, letterSpacing: '0.2em', color: C.dim, textTransform: 'uppercase', marginBottom: 6 }}>
        {label}
      </div>
      <div style={{
...
function FeedRow({ text, ts, fresh }: { text: string; ts: string; fresh: boolean }) {
  return (
    <div style={{
      ...
      animation: fresh ? 'feedSlide 0.35s ease' : 'none',
      opacity: fresh ? 1 : 0.65,
    }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', flex: 1, minWidth: 0 }}>
        ...
        <span style={{ fontSize: 11, color: C.text, fontFamily: C.mono, lineHeight: 1.5, wordBreak: 'break-word' }}>
          {text}
        </span>
      </div>
...
function TradeRow({ trade }: { trade: Trade }) {
  ...
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize: 10, color: C.dim, marginBottom: 2, fontFamily: C.mono }}>
          {shortenAddr(trade.id)}
        </div>
        <div style={{ fontSize: 12, color: C.accent, fontFamily: C.mono, fontWeight: 600 }}>
          {formatEth(trade.amount)} ETH
        </div>
      </div>
      <span style={{
      ...
      <div style={{ fontSize: 9, color: overdue ? C.danger : C.dim, fontFamily: C.mono, textAlign: 'right' }}>
        {overdue ? '⚡ OVERDUE' : new Date(trade.deadline * 1000).toLocaleDateString()}
      </div>
    </div>
  );
}
    ...
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ height: 3, background: 'rgba(255,255,255,0.06)', borderRadius: 2, overflow: 'hidden' }}>
        <div style={{
          height: '100%',
          width: `${pct}%`,
          background: `linear-gradient(90deg, ${C.accent}, rgba(184,255,0,0.4))`,
          borderRadius: 2,
...
interface GlobalCommandProps {
  account: string;
  trades: Trade[];
  onEnterLedger: () => void; // navigate to main ledger (Screen 1 app)
}
...
export default function GlobalCommand({ account, trades, onEnterLedger }: GlobalCommandProps) {
  ...
  const [tab,         setTab]         = useState<'trades'|'nodes'>('trades');
  ...
  // Live feed pump — very light, one update every 4s
  useEffect(() => {
  ...
  // Region volumes from globe nodes
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', color: C.accent }}>
                IRON LEDGER // COMMAND CENTER
              </span>
            </div>
            <span style={{ fontSize: 9, color: C.dim, letterSpacing: '0.1em' }}>
              SCREEN 02 — GLOBAL SETTLEMENT OVERVIEW
            </span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 10, color: C.dim, fontFamily: C.mono }}>
              {account ? `OP: ${shortenAddr(account)}` : 'NOT CONNECTED'}
            </span>
            <button
              className="gc-btn"
              onClick={onEnterLedger}
              style={{
                padding: '6px 16px',
        ...
        <div style={{
          ...
          {[
            ...
            <div key={label} style={{
              ...
              <div style={{ fontSize: 20, fontFamily: C.serif, fontWeight: 700, color: accent, lineHeight: 1, marginBottom: 4 }}>
                {value}
              </div>
              <div style={{ fontSize: 9, letterSpacing: '0.18em', color: C.dim, textTransform: 'uppercase' }}>
                {label}
              </div>
            </div>
          ))}
            ...
            <div style={{
              ...
              {hoveredNode && (
                <div style={{
                  ...
                  <div style={{ color: C.accent, fontWeight: 700, marginBottom: 4, letterSpacing: '0.1em' }}>
                    {hoveredNode.city}
                  </div>
                  <div style={{ color: C.dim, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 16px', fontSize: 10 }}>
                    ...
                    <span>Status</span><span style={{ color: hoveredNode.active ? C.accent : C.orange }}>
                      {hoveredNode.active ? 'ONLINE' : 'STANDBY'}
                    </span>
                  </div>
                </div>
              )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              {tab === 'trades' ? (
                trades.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '40px 20px', color: C.dim, fontSize: 12 }}>
                    ...
                    <span
                      onClick={onEnterLedger}
                      style={{ color: C.accent, cursor: 'pointer', textDecoration: 'underline', fontSize: 11 }}
                    >
                      Enter Ledger to create one ↗
                    </span>
                  </div>
                ) : (
                  <>
                    <div style={{ fontSize: 9, color: C.dim, letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 8 }}>
                      ▸ YOUR POSITIONS
                    </div>
                ...
                <>
                  ...
                  {GLOBE_NODES.sort((a,b) => b.volume - a.volume).map(node => (
                    ...
                    >
                      ...
                      <div style={{ fontSize: 10, color: C.accent, fontFamily: C.mono, textAlign: 'right' }}>
                        {node.volume.toFixed(1)} ETH
                      </div>
                      <div style={{
                        fontSize: 9,
                        color: node.active ? C.accent : C.orange,
                        textAlign: 'right',
                        fontWeight: 700,
                        letterSpacing: '0.08em',
                      }}>
                        {node.active ? 'LIVE' : 'STBY'}
                      </div>
                    </div>
                  ))}
                </>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ fontSize: 9, color: C.dim, letterSpacing: '0.2em', textTransform: 'uppercase' }}>
                  ▸ LIVE FEED
                </span>
              ...
              <div style={{
                ...
                {feedItems.map((item, i) => (
                  <FeedRow key={i} text={item.text} ts={item.ts} fresh={item.fresh} />
                ))}
              </div>
            </div>
          </div>
